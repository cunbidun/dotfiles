...
-- {{{ Key bindings
globalkeys = gears.table.join(awful.key({Modkey, 'Mod1'}, 'j', function()
  awful.client.incwfact(0.05)
end), awful.key({Modkey, 'Mod1'}, 'k', function()
  awful.client.incwfact(-0.05)
end), awful.key({Modkey, 'Shift'}, 's', function()
  ...
  group = 'hotkeys'
}), awful.key({}, 'XF86AudioMute', volume_widget.toggle, {description = 'toggle mute', group = 'hotkeys'}), awful.key(
                                  {Modkey}, '\\', naughty.destroy_all_notifications,
  ...
  s.mywibox.visible = not s.mywibox.visible
                              ...
                              awful.key({Modkey}, 'Left', awful.tag.viewprev,
                                        {description = 'view previous', group = 'tag'}), awful.key({Modkey}, 'Right',
                                                                                                   awful.tag.viewnext, {
  ...
  awful.client.focus.byidx(-1)
end, {description = 'focus previous by index', group = 'client'}), awful.key({Modkey, 'Shift'}, 'j', function()
  awful.client.swap.byidx(1)
end, {description = 'swap with next client by index', group = 'client'}), awful.key({Modkey, 'Shift'}, 'k', function()
  awful.client.swap.byidx(-1)
end, {description = 'swap with previous client by index', group = 'client'}), awful.key({Modkey}, ',', function()
  awful.screen.focus_relative(1)
end, {description = 'focus the next screen', group = 'screen'}), awful.key({Modkey}, '.', function()
  awful.screen.focus_relative(-1)
end, {description = 'focus the previous screen', group = 'screen'}), awful.key({Modkey}, 'u',
...
end, {description = 'open a terminal', group = 'launcher'}), awful.key({Modkey, 'Control'}, 'r', awesome.restart, {
  ...
  awful.tag.incncol(-1, nil, true)
end, {description = 'decrease the number of columns', group = 'layout'}), awful.key({Modkey}, 'i', function()
  awful.layout.inc(1)
end, {description = 'select next', group = 'layout'}), awful.key({Modkey, 'Shift'}, 'i', function()
  awful.layout.inc(-1)
end, {description = 'select previous', group = 'layout'}), awful.key({Modkey, 'Control'}, 'n', function()
  local c = awful.client.restore()
  -- Focus restored client
  if c then c:emit_signal('request::activate', 'key.unminimize', {raise = true}) end
end, {description = 'restore minimized', group = 'client'}), awful.key({Modkey}, 'p', function()
  menubar.show()
...
end, {description = 'move to master', group = 'client'}), awful.key({Modkey, 'Shift'}, '.', function(c)
  c:move_to_screen()
