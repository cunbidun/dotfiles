...
static void (*handler[LASTEvent])(const XEvent *) = {
    [ButtonPress] = buttonpress,   [ClientMessage] = clientmessage, [ConfigureNotify] = configurenotify, [ConfigureRequest] = configurerequest,
    [CreateNotify] = createnotify, [UnmapNotify] = unmapnotify,     [DestroyNotify] = destroynotify,     [Expose] = expose,
    [FocusIn] = focusin,           [KeyPress] = keypress,           [MapRequest] = maprequest,           [PropertyNotify] = propertynotify,
};
static int bh, wx, wy, ww, wh, vbh;
static unsigned int numlockmask;
static Bool running = True, nextfocus, doinitspawn = True, fillagain = False, closelastclient = False, killclientsfirst = False;
static Display *dpy;
static DC dc;
...
char *getatom(int a) {
  ...
  XGetWindowProperty(dpy, win, wmatom[a], 0L, BUFSIZ, False, XA_STRING, &adummy, &idummy, &ldummy, &ldummy, &p);
  if (p)
...
void manage(Window w) {
  updatenumlockmask();
  {
    ...
    for (i = 0; i < LENGTH(keys); i++) {
      if ((code = XKeysymToKeycode(dpy, keys[i].keysym))) {
        for (j = 0; j < LENGTH(modifiers); j++) {
          XGrabKey(dpy, code, keys[i].mod | modifiers[j], w, True, GrabModeAsync, GrabModeAsync);
        }
      }
    }
    ...
    if (nclients > 1 && nextpos < nclients - 1)
      memmove(&clients[nextpos + 1], &clients[nextpos], sizeof(Client *) * (nclients - nextpos - 1));
...
void propertynotify(const XEvent *e) {
  ...
  if (ev->state == PropertyNewValue && ev->atom == wmatom[WMSelectTab]) {
  ...
  } else if (ev->state == PropertyNewValue && ev->atom == XA_WM_HINTS && (c = getclient(ev->window)) > -1 && (wmh = XGetWMHints(dpy, clients[c]->win))) {
    ...
    XFree(wmh);
  } else if (ev->state != PropertyDelete && ev->atom == XA_WM_NAME && (c = getclient(ev->window)) > -1) {
    updatetitle(c);
  }
}
...
void setup(void) {
  ...
  attrs.event_mask =
      FocusChangeMask | KeyPressMask | ExposureMask | VisibilityChangeMask | StructureNotifyMask | ButtonMotionMask | ButtonPressMask | ButtonReleaseMask;
  attrs.background_pixmap = None;
  attrs.colormap          = cmap;

  win = XCreateWindow(dpy, root, wx, wy, ww, wh, 0, 32, InputOutput, visual, CWBackPixmap | CWBorderPixel | CWBitGravity | CWEventMask | CWColormap, &attrs);
  ...
  XSelectInput(dpy, win,
               SubstructureNotifyMask | FocusChangeMask | ButtonPressMask | ExposureMask | KeyPressMask | PropertyChangeMask | StructureNotifyMask |
...
void updatetitle(int c) {
  if (!gettextprop(clients[c]->win, wmatom[WMName], clients[c]->name, sizeof(clients[c]->name)))
