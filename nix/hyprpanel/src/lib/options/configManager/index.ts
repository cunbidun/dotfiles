...
import tsDefaults from 'src/configuration/tsDefaults';
...
export class ConfigManager {
    ...
    public readConfig(): Record<string, unknown> {
        const baseConfig = this._cloneObject(tsDefaults);
        ...
        const diskConfig = this._parseConfigSafely(fileContent);
        return this._mergeObjects(baseConfig, diskConfig);
    }
    ...
    private _cloneObject<T>(value: T): T {
        return JSON.parse(JSON.stringify(value));
    }

    private _mergeObjects(
        baseObject: Record<string, unknown>,
        overrideObject: Record<string, unknown>,
    ): Record<string, unknown> {
        const merged: Record<string, unknown> = this._cloneObject(baseObject);

        for (const [key, overrideValue] of Object.entries(overrideObject)) {
            const currentValue = merged[key];

            if (this._isValidObject(currentValue) && this._isValidObject(overrideValue)) {
                merged[key] = this._mergeObjects(currentValue, overrideValue);
            } else {
