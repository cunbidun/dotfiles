...
export class WallpaperService extends GObject.Object {
    ...
    constructor() {
        ...
        this.wallpaper = options.wallpaper.image.get();
        ...
        options.wallpaper.image.subscribe(() => {
            if (options.wallpaper.enable.get() && this._daemon.isRunning) {
                this._wallpaper();
            }
        });
    }
    ...
    private _wallpaper(): void {
        ...
        if (!(GLib.file_test(wallpaperPath, EXISTS) && GLib.file_test(wallpaperPath, IS_REGULAR))) {
            console.warn(`Cannot set wallpaper: invalid path '${wallpaperPath}'`);
            return;
        }
    ...
    /**
     * Applies a wallpaper path directly without writing a local cache file.
     *
     * @param path - Path to the wallpaper image file
     */
    private async _setWallpaper(path: string): Promise<void> {
        ...
        try {
            options.wallpaper.image.set(path);
            this._wallpaper();
        } catch (error) {
