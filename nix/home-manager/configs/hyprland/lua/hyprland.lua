...
package.path = configHome .. "/hypr/?.lua;" .. configHome .. "/hypr/?/init.lua;" .. package.path

local command = require("user.command")
local plugin = require("user.plugin")
local theme = require("user.theme")
local mainMod = "SUPER"
local colors = theme.colors()

hl.plugin.load(plugin.hyprfocus)
...
local windowRules = {
	...
	{ match = { class = "^(zoom)$", title = "^(menu window|confirm window|annotate_toolbar|as_toolbar|zoom_linux_float_video_window)$" }, float = true },
	{ match = { class = "^(zoom)$", title = "^(menu window|confirm window|annotate_toolbar|as_toolbar|zoom_linux_float_video_window)$" }, decorate = false },
	{ match = { class = "^(zoom)$", title = "^(menu window|confirm window)$" }, stay_focused = true },
	{ match = { class = "^(zoom)$", title = "^(Participants.*)$" }, float = true },
	{ match = { class = "^(zoom)$", title = "^(Participants.*)$" }, decorate = false },
...
hl.bind("XF86AudioRaiseVolume", exec(command.increase_volume), { locked = true, repeating = true })
hl.bind("XF86AudioLowerVolume", exec(command.decrease_volume), { locked = true, repeating = true })
hl.bind("XF86AudioMute", exec(command.toggle_volume), { locked = true, repeating = true })
hl.bind("XF86AudioPlay", exec(command.playerctl .. " play-pause"), { locked = true })
hl.bind("XF86AudioPause", exec(command.playerctl .. " play-pause"), { locked = true })
hl.bind("XF86AudioNext", exec(command.playerctl .. " next"), { locked = true })
hl.bind("XF86AudioPrev", exec(command.playerctl .. " previous"), { locked = true })
...
for key, workspace in pairs(projectKeys) do
	hl.bind(mainMod .. " + " .. key, exec(command.wsctl .. " project " .. workspace))
	hl.bind(mainMod .. " + SHIFT + " .. key, exec(command.wsctl .. " project-move " .. workspace))
end
...
hl.define_submap("ws", function()
	for i = 1, 5 do
		hl.bind(tostring(i), exec_and_reset(command.wsctl .. " goto " .. i))
		hl.bind("SHIFT + " .. i, exec_and_reset(command.wsctl .. " move " .. i))
	end
	hl.bind("w", exec_and_reset(command.wsctl .. " main"))
	hl.bind("SHIFT + w", exec_and_reset(command.wsctl .. " main-move"))
	hl.bind("escape", hl.dsp.submap("reset"))
end)
