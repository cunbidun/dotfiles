...
function volume:toggle()
	volume:_cmd("toggle_volume")
end

function volume:raise()
	volume:_cmd("increase_volume")
end

function volume:lower()
	volume:_cmd("decrease_volume")
end

local function parse_output(stdout)
	local level = string.match(stdout, "(%d?%d?%d)%%")
	if stdout:find("%[M%]") then
...
local function worker(args)
    ...
    -- notif("")
		...
		spawn.easy_async("get_volume", function(stdout, stderr, exitreason, exitcode)
